...
if phase == "Phase 0: Data Harmonization":
    ...
    st.markdown("Ingest raw CSVs, standardize columns, and **strictly clean** the data.")
    ...
    with col1:
        ...
        scan_btn = st.button("🔍 Scan Files", type="primary", use_container_width=True, key="scan_files_btn_p0")
    ...
    if scan_btn or st.session_state.get('scan_done'):
        ...
        if not health_df.empty:
            ...
            drop_percentage = (total_dirty_rows / total_raw_rows) * 100 if total_raw_rows > 0 else 0
            
            # 1. METRICS ROW
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Total Rows (Raw)", f"{total_raw_rows:,.0f}")
            m2.metric("Rows to Keep", f"{total_raw_rows - total_dirty_rows:,.0f}", help="Rows with 100% complete data.")
            m3.metric("Rows to Drop", f"{total_dirty_rows:,.0f}", delta="-Dirty", delta_color="inverse", help="Rows with at least one missing value.")
            m4.metric("Data Loss %", f"{drop_percentage:.2f}%", delta_color="inverse")
            
            # Show Per-File Breakdown
            with st.expander("📄 View Per-File Breakdown (Click to Expand)", expanded=True):
            ...
            if melt_cols:
                melted = health_df.melt(id_vars=["File Name"], value_vars=melt_cols, var_name="Column", value_name="Count")
                melted["Column"] = melted["Column"].str.replace("Missing ", "")
                melted = melted[melted["Count"] > 0]
                
                if not melted.empty:
                    fig = px.bar(melted, x="File Name", y="Count", color="Column", title="Missing Values Distribution", height=400)
            ...
            if total_dirty_rows == 0:
                ...
                st.warning(f"⚠️ You are about to drop {total_dirty_rows:,.0f} rows ({drop_percentage:.2f}% of data).")
    ...
    # --- PART 4: DASHBOARDS ---
    if st.session_state.get('processing_done') or (Path("data/processed/final_aggregated_rides.csv").exists()):
        st.divider()
        st.subheader("4. Analytical Dashboards")
        
        subtab1, subtab2, subtab3 = st.tabs(["📈 Diagnostics (Trends)", "📊 Comprehensive (Stats)", "🌍 Geographic (Map)"])
        
        # 1. Diagnostics Tab
        with subtab1:
            st.subheader("System Diagnostics")
            st.markdown("*High-level health check: Is the system growing? When do people ride?*")
            fig_diag = pipeline.get_diagnostics_dashboard()
            if fig_diag:
                st.plotly_chart(fig_diag, use_container_width=True)
                st.info("""**Reading the Charts:**\n* **Top Chart (The Pulse):** Shows the 7-day rolling average.\n* **Bottom Chart (The Heartbeat):** Shows average activity per hour.""")
                st.markdown("""# 📉 Conclusion: Temporal Constraints for Optimization\n\n### 1. The Volatility Gap\n**Figure 1 (The Pulse)** reveals high seasonal volatility...\n\n### 2. The Operational Window\n**Figure 2 (The Heartbeat)** shows the daily demand cycle...""")
            else:
        ...
        with subtab2:
            st.subheader("Comprehensive Analytics")
            st.markdown("""# Seasonal Dynamics & Product Adoption\n\n**Objective:** Evaluate the long-term structural shifts...""")
            fig_main = pipeline.get_comprehensive_dashboard()
            if fig_main:
                st.plotly_chart(fig_main, use_container_width=True)
                st.markdown("""# 📉 Strategic Insights\n\n### 1. The "Base Load" Stability\n**Chart 1** reveals that **Members** (Blue) form the resilient core...\n\n### 2. Growth Validation\n**Chart 2** confirms a consistent upward trajectory...\n\n### 3. The E-Bike Revolution\n**Chart 3** highlights a critical operational challenge...""")
            else:
        ...
        with subtab3:
            st.subheader("Geographic Insights")
            st.markdown("""# Geographic Distribution & Infrastructure\n\n**Objective:** Validate the assumption that Manhattan...""")
            fig_geo = pipeline.get_geographic_dashboard()
            if fig_geo:
                st.plotly_chart(fig_geo, use_container_width=True)
                st.markdown("""# 🌍 Geographic Insights\n\n### 1. Manhattan Dominance\nThe charts confirm that **Manhattan (Blue)** is operating...""")
            else:
...
# PHASE 1: NETWORK DIAGNOSTICS (ADDED)
    ...
    **The Tidal Wave Hypothesis:** We hypothesize that the system failure is driven by a massive, unidirectional flow 
    ...
    # Initialize
    analyzer = DiagnosticsAnalyzer(raw_dir="data/raw", processed_dir="data/processed")
