...
#     """
#     Phase 4: Pure XGBoost Engine
#     Focus: High Accuracy + Explainability (Feature Importance)
#     """
...
#         stats = self.df.groupby('station_name').agg({'starts': 'sum', 'returns': 'sum'}).reset_index()
#         stats['volume'] = stats['starts'] + stats['returns']
#         return stats.sort_values('volume', ascending=False)['station_name'].tolist()

#     def _fetch_weather(self, start_date="2023-01-01", end_date="2025-11-30"):
#         """Fetches REAL historical weather."""
...
#     def run_forecast(self, station_name):
#         """Runs XGBoost and returns Data Preview + Feature Importance."""
...
#         station_df['month_key'] = pd.to_datetime(station_df['month_key']).dt.to_period('M').astype(str)
#         merged = daily.merge(station_df[['month_key', 'starts', 'returns']], on='month_key', how='left').ffill().bfill()
        
#         # Normalize (Monthly -> Daily)
...
#         merged['daily_starts'] = merged.apply(lambda r: apply_noise(r, r['starts']), axis=1).clip(lower=0)
#         merged['daily_returns'] = merged.apply(lambda r: apply_noise(r, r['returns']), axis=1).clip(lower=0)
        
#         # --- 2. FEATURE ENGINEERING ---
#         merged['dow'] = merged['date'].dt.dayofweek
#         merged['month'] = merged['date'].dt.month
#         merged['month_sin'] = np.sin(2 * np.pi * merged['month']/12) # Cyclical Time
#         merged['is_holiday'] = self._get_holidays(merged['date'])
        
#         # Rolling Averages (The "Trend" Features)
...
#         features_demand = ['daily_starts_roll7', 'dow', 'month_sin', 'temp_f', 'is_rain', 'is_holiday']
#         features_supply = ['daily_returns_roll7', 'dow', 'month_sin', 'temp_f', 'is_rain', 'is_holiday']
        
#         # --- 4. TRAINING XGBOOST ---
#         # Demand Model
#         model_d = xgb.XGBRegressor(n_estimators=500, learning_rate=0.03, max_depth=5, n_jobs=-1)
#         model_d.fit(train[features_demand], train['daily_starts'])
        
#         # Supply Model
#         model_s = xgb.XGBRegressor(n_estimators=500, learning_rate=0.03, max_depth=5, n_jobs=-1)
#         model_s.fit(train[features_supply], train['daily_returns'])
        
#         # --- 5. PREDICTION ---
#         # Predict on WHOLE dataset for plotting continuity
...
#         # --- 6. EXPLAINABILITY ---
#         # Extract Feature Importance (Importance Type = 'gain')
#         imp_d = model_d.feature_importances_
#         imp_s = model_s.feature_importances_
        
#         # Average importance across Supply/Demand for a general view
...
#         # Return Dataset Preview (Subset of columns for display)
#         preview_cols = ['date', 'daily_starts', 'daily_returns', 'daily_starts_roll7', 'temp_f', 'is_rain', 'is_holiday']
#         dataset_preview = merged[preview_cols].tail(10) # Show last 10 rows

#         return merged, metrics, importance_df, dataset_preview

#     def plot_forecast(self, df):
#         """Visualizes XGBoost Forecast."""
#         fig = make_subplots(
...
#             x=df['date'], y=df['actual_net_flow'],
#             mode='lines', name='Actual Reality',
...
#         fig.add_trace(go.Scatter(
#             x=train_df['date'], y=train_df['pred_net_flow'],
#             mode='lines', name='Model Fit',
#             line=dict(color='#007AFF', width=2)
#         ), row=1, col=1)
        
#         fig.add_trace(go.Scatter(
#             x=test_df['date'], y=test_df['pred_net_flow'],
...
#             x=test_df['date'], y=test_df['error'],
#             name='Error', marker_color=colors
#         ), row=2, col=1)
